...
class Publication {
    ...
    this.observeHandle = this.cursor.observe({
      added: Meteor.bindEnvironment((doc) => {
        ...
        if (alreadyPublished) {
          ...
          if (this.hasChildrenDepend(Object.keys(doc))) {
      ...
      changed: Meteor.bindEnvironment((newDoc, oldDoc) => {
        ...
        const changes = DiffSequence.makeChangedFields(newDoc, oldDoc)
        if (this.hasChildrenDepend(Object.keys(changes))) {
  ...
  hasChildrenDepend = function hasChildrenDepend (keysArr) {
    debugLog('Publication.hasChildrenDepend', this._getCollectionName())
    if (!this.childrenDepend.length) {
      return true
    }
    const releventChangedFields = [keysArr, this.childrenDepend].reduce((a, b) => a.filter(c => b.includes(c)))
